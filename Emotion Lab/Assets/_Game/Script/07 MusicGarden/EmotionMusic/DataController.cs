...
namespace Barnabus.EmotionMusic
{
    public class DataController : MonoBehaviour
    {
        ...
        private Character GetCharacterAsset(int characterID)
        {
            return sheetController.Asset.GetCharacterAssetByID(characterID);
        }
        ...
        public void SaveData<T>(T data, string fileName = "", bool isForceSaveNewFile = true)
        {
            if (currentDataIndex == -1 || isSaveAsNew || (fileName != "") && isForceSaveNewFile)
            {
                ...
                currentDataIndex = DataStream.DataAccess.AddData<T>(key, new DataStream.Data<T>(currentDataName, data), GameSettings.DataType, GameSettings.AccessType);
            }
            else DataStream.DataAccess.ModifyData<T>(key, currentDataIndex, new DataStream.Data<T>(currentDataName, data), GameSettings.DataType, GameSettings.AccessType);
        ...
        {
            DataStream.DataList<EM_Data> dataList = DataStream.DataAccess.LoadDataList<EM_Data>(key, GameSettings.DataType, GameSettings.AccessType);
            ...
            for (int i = 0; i < dataInfoList.Count; i++)
            {
                ...
                for(int j = 0; j < dataList[i].data.charactersID.Count; j++)
                {
                    dataInfoList[i].SetCharacter(j, GetCharacterAsset(dataList[i].data.charactersID[j]));
                }
            }
        }
        ...
        {
            DataStream.Data<T> data = DataStream.DataAccess.LoadDataList<T>(key, GameSettings.DataType, GameSettings.AccessType)[selectedInfoID];
        ...
        {
            DataStream.DataAccess.RemoveData<T>(key, selectedInfoID, GameSettings.DataType, GameSettings.AccessType);
        ...
        public void RenameSelectedData<T>()
        {
            ...
            DataStream.DataAccess.RenameData<T>(key, selectedInfoID, dataInfoList[selectedInfoID].GetNewName(), GameSettings.DataType, GameSettings.AccessType);
