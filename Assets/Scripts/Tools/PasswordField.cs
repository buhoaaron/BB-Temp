...
namespace Barnabus.UI
{
    ...
    /// </summary>
    public class PasswordField : Selectable,
        ...
        IScrollHandler
    {
        ...
        private bool m_IsInsertChar = false;
            ...
            if (m_TextComponent != null && m_TextComponent.font != null && m_PreventCallback == false)
            {
                ...
                if (inputType == InputType.Password)
                {
                    if (fullText.Length > 0)
                    {
                        if (m_IsInsertChar)
                        {
                            var caretPositionIndex = caretPosition - 1;

                            for (int index = 0; index < fullText.Length; index++)
                            {
                                var isCurrent = index == caretPositionIndex;
                                processed += isCurrent ? fullText[index] : asteriskChar;
                            }
                        }
                        else
